...
import "./Vesting-Test.sol";

contract VestingSpawnerTest is Ownable {
    ...
    function spawnVestingContract(
        ...
        require(_startTime >= 0, "VESTING-SPAWNER-START-TIME-TOO-EARLY"); // TODO FIX BACK
        ...
        VestingTest(newVestingContract).initialize(
            _recipient,
            address(xgt),
            _startTime,
            _epochDuration,
    ...
    function multiClaim(uint256 _from, uint256 _to) external {
        ...
        for (uint256 i = _from; i <= _to; i++) {
            if (VestingTest(vestingContracts[i]).hasClaim()) {
                VestingTest(vestingContracts[i]).claim();
            }
        }
    }
